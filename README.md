...
- ✅ **一键更新** - 批量更新版本不一致的实例
- ✅ **收编较新副本** - 副本自更新后版本反超原版时，把它提升为新原版（adopt），避免被降级
- ✅ **交互式菜单** - 友好的命令行界面（人类入口）
...
启动时会自动检测版本差异，发现实例版本与原始微信不一致时提示一键同步（较新的副本走 adopt、较旧的走 update）。
...
double-wechat adopt  [<0-9>] [--yes]                # 收编自更新后版本反超原版的副本
double-wechat help
...
- 写操作（create/delete/update/adopt）默认会要 stdin 确认；脚本/AI 调用必须加 `--yes`
...
| `/double-wechat:adopt [n]` | 把自更新后版本反超原版的副本提升为新原版 |
...
> **不要给子命令加 sudo**。`double-wechat create / delete / update / start / adopt` 会主动拒绝 `EUID == 0` 退出。原因：root 创建的副本归 root 所有，后续无 sudo 操作（包括日常的覆盖更新）会失败——这正是上面"一次性迁移"想消除的痛点。子命令设计为完全无需特权，正常调用即可。

### 副本自更新与 `adopt`

副本被启动后，微信自带的应用内更新器可能就地替换整个 `.app`——把 build 升高、`CFBundleIdentifier` 改回原版的 `com.tencent.xinWeChat`、签名换回腾讯签名。结果这个副本版本反而高于原始 `WeChat.app`，且丢失了多开身份。

此时不能用 `update`（它会拿较旧的原版覆盖、把副本**降级**）。`adopt` 反向处理：

1. 校验该副本确实比原版新、且是带正规腾讯签名的干净官方包
2. 用该副本覆盖原始 `WeChat.app`（升级原版，不降级）
3. 从新原版重新打包该副本，恢复 `com.tencent.xinWeChat<n>` 标识符与 adhoc 签名

`adopt` 是本工具唯一会写原始 `WeChat.app` 的命令；交互菜单的「一键同步」与启动自检会在需要时自动先 adopt 再 update。
...
- update / delete / 覆盖 create / adopt（重打包被收编的副本）都会让对应实例的登录态丢失，需要重新扫码登录
